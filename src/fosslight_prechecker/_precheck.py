...
_USER_EXCLUDE_LIST = []  # Exclude paths from checking
...
def find_oss_pkg_info_and_exclude_file(path):
    ...
    try:
        for root, dirs, files in os.walk(path):
            if os.path.abspath(root) in _USER_EXCLUDE_LIST:
                continue
            for dir in dirs:
                # For hidden folders
                dir_path = os.path.join(root, dir)
                dir_abs_path = os.path.abspath(dir_path)
                if any(os.path.commonpath([dir_abs_path, exclude_path]) == exclude_path for exclude_path in _USER_EXCLUDE_LIST):
                    continue
                if dir.startswith("."):
            ...
            for file in files:
                ...
                if any(os.path.commonpath([file_abs_path, exclude_path]) == exclude_path for exclude_path in _USER_EXCLUDE_LIST):
                    continue
...
def filter_missing_list(missing_list):
    ...
    for file in missing_list:
        ...
        if not any(os.path.commonpath([abs_path, path]) == path for path in _USER_EXCLUDE_LIST):
...
def set_exclude_list(path_to_find, exclude_path):
    global _USER_EXCLUDE_LIST

    for path in exclude_path:
        if path.strip != "":
            _USER_EXCLUDE_LIST.append(os.path.abspath(os.path.join(path_to_find, path)))
